...
//			Arrays.fill(edibleTimes,(int)(G.EDIBLE_TIME*(Math.pow(G.EDIBLE_TIME_REDUCTION,totLevel))));						
	...
	public int getNodeUsingXY(int x, int y)
	{
		// Check if X and the Y correspond a particular Node by getting the exact or the nearest Node
		int minDiff = 9999;
		int tempDiff = 9999;
		int nearestNodeIndex = 1;

		for(int index = 0; index < mazes[curMaze].graph.length; index++)
		{
			tempDiff = Math.abs(x - mazes[curMaze].graph[index].x) + Math.abs(y - mazes[curMaze].graph[index].y);
			//System.out.println("X: " + x + ", getX(index): " + getX(index) + ", y: " + y + ", getY(index)" + getY(index) + ", tempDIff" + tempDiff);
		...
		//System.out.println("MinDIff" + minDiff + ", NearestNode: " + nearestNodeIndex);
